...
def tree_detail():
    ...
    surname_filter = request.args.get('surname', '').strip()  # пустая строка = все

    # Базовый запрос всех персон дерева
    persons_query = Person.query.filter_by(tree_id=tree.id).order_by(Person.surname, Person.name)
    all_persons = persons_query.all()

    # Собираем список уникальных фамилий для фильтра
    surnames = sorted(list({p.surname for p in all_persons}))

    # Применяем фильтр, если задан и не пуст
    ...
    if view == 'tree':
        nodes = []
        edges = []
        # Если задан фильтр, оставляем только персон с нужной фамилией
...
def confirm_person():
    ...
    db.session.commit()
    flash('Персона добавлена', 'success')
    return redirect(url_for('person.person_detail', person_id=parent_id if parent_id else person.id))
